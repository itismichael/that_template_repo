...
def test_cli_main_default():
    ...
    assert "Core library says: Hello, World! This greeting comes from 'your_core_library'." in result.stdout
...
def test_cli_main_custom_name():
    ...
    assert "Core library says: Hello, Cascade! This greeting comes from 'your_core_library'." in result.stdout
...
def test_cli_main_custom_count():
    ...
    assert result.exit_code == 0
    assert "Hello World!" in result.stdout
    core_message = "Core library says: Hello, World! This greeting comes from 'your_core_library'."
...
def test_cli_help():
    ...
    assert "Say hello and demonstrate core library functionality." in result.stdout # MODIFIED: Check for command description
